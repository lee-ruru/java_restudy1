...
public class Sample2 {
	...
	public static void main(String[] args) {
		...
		생성자 오버로딩
		생성자(Constructor)
		다음 클래스들은 이전 챕터들을 진행하며 만든 클래스들이다. 만약 아래의 클래스가 현재 만들어지지 않았다면 작성한 후에 따라해 보자.
		class Animal {
		    String name;
		...
		Animal, Dog, HouseDog 클래스들이 준비 되었다면 이제 main메소드를 다음과 같이 수정한 후 실행 해 보자.
		(... 생략)
		public class Sample {
		    public static void main(String[] args) {
		...
		이렇듯 HouseDog 클래스는 코딩하기에 따라 객체변수 name에 값을 설정할 수도 있고 설정 안할 수도 있다. 그렇다면 
		name 이라는 객체변수에 값을 무조건 설정해야만 객체가 생성될 수 있도록 강제할 수 있는 방법은 없을까?
		...
		첫번째 코드와 두번째 코드의 차이점은 무엇일까? 두번 째 코드에는 생성자가 구현되어 있다. 생성자의 입력 항목이 없고 생성자 
		내부에 아무 내용이 없는 위와 같은 생성자를 디폴트 생성자라고 부른다.
		위와 같이 디폴트 생성자를 구현하면 new Dog() 로 Dog 클래스의 객체가 만들어 질 때 위에 구현한 디폴트 생성자가 실행될 것이다.
		만약 클래스에 생성자가 하나도 없다면 컴파일러는 자동으로 위와같은 디폴트 생성자를 추가한다. 하지만 사용자가 작성한 생성자가 하나라도 구현되어
		있다면 컴파일러는 디폴트 생성자를 추가하지 않는다.
