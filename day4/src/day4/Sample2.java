...
public class Sample2 {
	...
	public static void main(String[] args) {
		...
		보통 함수와 메소드가 공존하는 언어(예:파이썬)에서는 두 개를 구분하여 말하기도 하지만 자바는 
		보통 메소드와 함수를 구분하여 말하지 않는다. 다만 자바에서 사용되는 함수의 정확한 명칭은 메소드이다.
		여기까지 공부해 오면서 메소드에 대해서 이미 알아 보았지만 이번 장에서는 좀 더 자세하게 알아보자.
		메소드를 설명하기 전에 믹서기를 생각해보자. 우리는 믹서기에 과일을 넣는다. 
		그리고 믹서를 이용해서 과일을 갈아서 과일 쥬스를 만들어 낸다. 우리가 믹서기에 넣는 과일은 입력이 되고 과일
		 쥬스는 그 출력(리턴값)이 된다.
		그렇다면 믹서기는 무엇인가?
		...
		Sample 클래스의 main 메소드에서 Sample sample = new Sample()처럼 자기 자신의 객체를 
		생성했다. 이것은 당연히 가능한 코드이고 작성한 클래스를 단독으로 실행시켜 테스트할 때 자주 사용하는 방법이다.
		...
		우리는 공부하면서 원서를 보기도 하고 누군가의 번역본을 보기도 하면서 의미는 같지만 표현이 다른 용어를 자주 
		만나게 된다. 한 예로 입력값을 다른 말로 메소드의 인수, 매개변수 등으로 말하기도 하고 결괏값을 출력값, 
		반환 값, 돌려주는 값 등으로 말하기도 한다. 이렇듯 많은 용어가 여러 가지 다른 말로 표현되지만 의미는 동일한
		경우가 많다. 따라서 이런 용어를 기억해 놓아야 머리가 덜 아플 것이다.
		...
		왜냐하면 sum 메소드의 리턴타입이 int로 되어 있기 때문이다. sum 메소드의 리턴타입은 int sum(int a, int b) { ... 에서 
		보듯이 메소드명(sum) 바로 좌측에 표기하도록 되어 있다.
		입력값이 없는 메소드
		...
		say 메소드를 쓰기 위해서는 say()처럼 괄호 안에 아무런 값도 넣지 않고 써야 한다. say 메소드는 입력값은 없지만 리턴값으로 'Hi'라는 문자열을 돌려준다.
		...
		3과 4의 합은 7입니다.
		아마도 여러분은 다음과 같은 질문을 할지도 모른다. "3과 4의 합은 7입니다." 라는 문장을 출력해 주었는데 
		왜 리턴값이 없다는 것인가? 이 부분이 초보자들이 혼동스러워 하는 부분이기도 한데 System.out.println 
		문은 메소드내에서 사용되어지는 문장일 뿐이다. 돌려주는 값은 당연히 없다. 
		...
		sayNick 메소드는 입력으로 받은 문자열 값을 받아서 출력하는 메소드이다. 이 메소드 역시 리턴값은 없다. 
		문자열을 출력한다는 것과 리턴값이 있다는 것은 전혀 다른 말이다. 혼동하지 말도록 하자. 메소드의 리턴값은 오로지 
		return문에 의해서만 생성된다. 이 메소드는 입력값으로 "fool" 이라는 값이 들어오면 문자열을 출력하지 않고 
		메소드를 즉시 빠져나간다. 이렇게 메소드 수행 시 특정 조건에 따라 메소드를 즉시 빠져나가고 싶은 경우 return 문을 
		이용하면 된다. 이 방법은 실제 프로그래밍에서 자주 쓰인다.
		...
		그리고 varTest 메소드를 입력 값 a를 주어 호출했다. 그 다음에 a의 값을 출력하게 하였다. 
		당연히 varTest 메소드에서 a의 값을 1만큼 증가시켰으니 2가 출력되어야 할 것 같지만 프로그램을 실행시켜 
		보면 1이라는 결과 값이 나온다.
		그 이유는 메소드 내에서 사용되는 변수는 메소드 안에서만 쓰여지는 변수이기 때문이다. 
		즉 public void varTest(int a) {라는 문장에서 입력 인수를 뜻하는 변수 a는 메소드 안에서만 
		쓰이는 변수이지 메소드 밖의 변수 a가 아니라는 말이다.
		...
		만약 varTest 입력값이 int 자료형이 아닌 객체였다면 얘기가 다르다. 객체를 메소드의 입력으로 넘기고 메소드가 객체의 속성값(객체변수 값)을
		 변경한다면 메소드 수행 이후에도 객체는 변경된 속성값을 유지한다. 이러한 차이가 나는 이유는 메소드에 전달하는 입력 자료형의 형태 때문인데 메소드에 값을 전달하느냐 아니면 객체를 전달하느냐에 따라 차이가 난다.
		...
		varTest 메소드는 입력으로 들어온 값을 1만큼 증가시켜 리턴한다. 따라서 a = sample.varTest(a)처럼
		 하면 a의 값은 다시 varTest 메소드의 리턴값으로 대입된다. (1만큼 증가된 값으로 a의 값이 변경된다.)
		...
		이전 예제에서는 a 라는 int 자료형 변수를 main 메서드에 선언했는데 위 예제에는 Sample 클래스의 
		객체변수로 선언했다. 그리고 varTest 메소드는 Sample 클래스의 객체를 입력받아 해당 객체의 객체변수 a의 
		값을 1만큼 증가시키는 역할을 하도록 수정했다. 그리고 main메소드에서는 varTest메소드에 1이라는 값을 
		전달하던것을 Sample 클래스의 객체인 sample을 넘기도록 수정했다.

		이렇게 수정하고 프로그램을 실행시켜보면 sample객체의 객체변수 a의 값이 원래는 1이었는데 varTest 메소드 
		실행 후 1만큼 증가되어 2가 출력되는 것을 확인할 수 있다.
		여기서 주목해야 하는 부분은 varTest 메소드의 입력 파라미터가 값이 아닌 Sample 클래스의 객체라는데 있다. 이렇게 메소드가 객체를 전달 받으면 메소드 내의 객체는 전달받은 객체 그 자체로 수행된다.
